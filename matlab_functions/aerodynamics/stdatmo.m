...
%
%   STDATMO(H,dT,Units) defines units for the input H and all outputs.
%   Options are SI (default) or US (a.k.a. Imperial, English). For SI, set
%   Units to [] or 'SI'. For US, set Units to 'US'. Input and output units
%   may be different by passing a cell array of the form {Units_in
%   Units_out}, e.g. {'US' 'SI'}. Keep in mind that dT is an offset, so
%   when converting between Celsius and Fahrenheit, use only the scaling
%   factor (dC/dF = dK/dR = 5/9). Units are as follows:
%       Input:                        SI (default)    US
%           H:     Altitude           m               ft
%           dT:    Temp. offset       degC/degK       degF/degR
%       Output:
...
%   V4.0    6 July 2011
%       Imperial temp offset now degF/degR instead of degC/degK