function [ P_bat_limited ] = ChargeLimiter(SoC_current, P_bat, params, plane)
...
%data.
    ...
    if(params.bat.chrg_lim_type == 1)
        % Model-based (theoretical) charge limiting curve
        
        % Max admissible charging power in [W] at any SoC
        P_bat_max_overall = params.bat.chrg_lim_Prel1 * params.bat.e_density * plane.bat.m /3600;
        
        if(SoC_current < params.bat.chrg_lim_SoC1)
            P_bat_limited = P_bat;
        ...
        elseif(SoC_current >= 1.0)
            ...
            c =-log(params.bat.chrg_lim_Prel2);

            P_bat_limited = exp (-c*(SoC_current-params.bat.chrg_lim_SoC1)/(1.0-params.bat.chrg_lim_SoC1))*P_bat_max_overall;
            if(P_bat < P_bat_limited)
                P_bat_limited = P_bat;
            end
        end    
    elseif(params.bat.chrg_lim_type == 2)
