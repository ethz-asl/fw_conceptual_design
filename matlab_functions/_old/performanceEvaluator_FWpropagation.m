...
%the real function
function [t_excess, t_endurance, t_chargemargin, min_SoC, flightdata, MDresults,t_sunrise,t_sunset] = performanceEvaluator_FWpropagation(...
...
if 1%eq_reached_flag == 1
    ...
    %Begin main loop
    while E_bat >=0 && h>=environment.h_0 && t<3600*48+t_eq
        ...
        if(mod(t,86400)>t_eq+1*3600 && mod(t,86400)<t_eq2-1*3600) 
            curr_turb =curr_turb + environment.turbulence_day; 
        end
        
        %Step1b: Calc Total electric power for level flight
        P_level = plane.P_prop_level * sqrt(plane.P_prop_level_rho/rho)*(1.0 + curr_turb);
        P_elec_tot = plane.P_avi+P_level;
        ...
        %charge or decharge?
        if delta_E>0
            delta_E = ChargeLimiter(E_bat/E_bat_max, delta_E/Dt * plane.bat.eta_chrg)*Dt;
        else
        ...
        P_charge_array = [P_charge_array, delta_E/Dt];
    end
    %--------------------------------------------
...
t_endurance = (t_end-t_sunrise)/3600;%max(24*3600-2*t_start_full,t_end-t_start_full)/3600;%(t_end-(t_eq-8.0*3600))/3600;
if t_excess > 0
    t_endurance = NaN; % the term endurance non-existent
end
i_min_E_Bat=find(t_array==(86400+t_eq),1,'first');
if(isempty(i_min_E_Bat)) 
    i_min_E_Bat=numel(t_array); 
end
...
flightdata.P_charge = P_charge_array;
...
% DEBUG OUTPUTS ONLY
%---------------------------------------------------------------------
if(DEBUG == 1)%|| environment.dayofyear==249 ||environment.dayofyear==252))
    str=strcat('24h flight overview: DayOfYear=',num2str(environment.dayofyear),' CLR=',num2str(environment.clearness));
    figure('Name',str);
    ax(1)=subplot(3,1,1);
    ...
    plotyy(t_array/3600,bat_array/E_bat_max,t_array/3600,bat_array/3600);
    ...
    plot(t_array/3600,P_solar_array,...
         ...
         t_array/3600,ones(1,numel(t_array))*plane.P_avi,...
         t_array/3600, P_charge_array);
    ylabel('Power [W] or [W/m^2]');
    xlabel('Time of Day [h]');
    legend('P_{Solar}[W]','P_{electot}[W]','P_{prop}[W]','P_{avionics}[W]','P_{charge}[W]');
    linkaxes(ax,'x');
    %DEBUG=0;
end
...
if(DEBUG==1)
    ...
    str=strcat(str, ' t_fc: ',num2str(t_fullycharged),' P_eltot: ',num2str(P_elec_tot),' Min. SoC: ',num2str(min_SoC));
    display(str);
end
